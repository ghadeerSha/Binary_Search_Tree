...
/* BST Operations */
    ...
    if (current->element == anElement)
        ...
        else if (anElement < current->element)
        {
            if (current->hasLeft())
                ...
                BSTNode<ElementType>* aNode = new BSTNode<ElementType>(anElement);
                current->left = aNode;
            }
            elementCount++;
            } 
    ...
    if(current->hasLeft())    //last left number 
      traverseInOrderR(visit, current->left);   //process current the left node 
    
    visit (current->element);    //going back after first processing and processes current node after the left node is checked 
    
    if(current->hasRight())       //goes to right number
      traverseInOrderR(visit,current->right);    //processes the current right node 
